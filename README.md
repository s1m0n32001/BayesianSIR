...
# BayesianSIR: Covid data analysis
...
They are drawn from probability distributions whose characteristic parameters ($b$ and $r$ respectively) change as the epidemic enters into a different stage. Thus, *change points* correspond to a shift in the average transmission/removal rates. This is summarized in the vector 
...
#### 2. Accept $\delta^{(g+1)} = \delta^\star$ with probability $p = \text{min}(1,\pi_{MH})$. If rejected, $\delta^{(g+1)} = \delta^{(g)}$. 
...
Finally, the set of $\delta^{(g)}$ obtained from the MC sampling is used to produce the Bayes estimator $\hat{\delta}$. Interpreting each $\delta^{(g)}$ as a cluster of time points $1, \dots, T$, $\hat{\delta}$ is obtained solving a clustering aggretion problem. 
...
Then, $\hat{\delta}$ is esteemed minimizing the following loss: $$\hat{\delta} = \underset{\delta}{\text{argmin}} \displaystyle\sum_{1\leq t < t' \leq T} \big\vert \mathbb{I}(\eta_t = \eta_{t'}) - \hat{q}_{tt'} \big\vert$$



#### 6. Simulated data

To evaluate performances of the explained method, we have simulated the Bayesian SIR model sampling $\Delta I$ and $\Delta R$ from the corresponding binomial distributions and then computing the three variables $S,\:I,\:R$. Below we present the $\texttt{Scenario\:1}$ proposed in the studied paper, having set as initial conditions $N=10^6,\: I_0=50$ and $T=100$ days.  

<p align="center">
  <img src="img/simulated_data.png" height="220"/>
</p>

The above plot is in perfect agreement with the one shown by *Gu & Yin*, as well as the one shown below where the simulated $\beta$'s and $\gamma$'s are compared with those imposed at the start of the simulation
...
Even in this case, there is very good agreement between the simulated and it was difficult to retrieve less pronounced change points, whereas the model is quite good at predicting sharp stage transitions. The numbers of daily confirmed and removed COVID-19 cases is here presented:

<p align="center">
  <img src="img/Confirmed_Removed.png" height="220"/>
</p>

$\beta$ and $\gamma$ are very well fitted, using the procedure afore explained, execption made for the starting points.

<p align="center">
  <img src="img/beta_gamma_real_df.png" height="220"/>
</p>

In our code, $\delta$ meets precisely the updating conditions described, but unfortunately we have not been able to reproduce all the 4 *change points*, but just 3 of them:

<p align="center">
  <img src="img/delta_Singapore.png" height="220"/>
</p>

As one can see by the above plot with $\beta$ the *transmission rate* is very high during the first several days of pandamic, then decreasing almost to zero. Instead, the $\gamma$ plot emphasizes how *removal rate* has two drops at the $\delta$ change points:

<p align="center">
  <img src="img/Transmission_Removal_Rates.png" height="220"/>
</p>


#### 8. Conclusion

Overall, we have been able to reproduce many of the results shown and to correct some mistakes made by the cited authors. Still some investigations must be carried out in order to understand with $\delta_t^{(g)}$ does not show the same behaviour presented in the paper, but only 2 of the main *change points*.